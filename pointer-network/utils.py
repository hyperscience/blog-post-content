...
def OCR(image_path: str) -> List[Dict[str, Any]]:
    ...
    image_data = image_data.loc[image_data.text.apply(lambda x: pd.notnull(x) and x != '')]
...
def display_doc(doc: Dict[str, Any], predicted_tokens: Optional[List[int]] = None):
    ...
    if predicted_tokens is None:
        ...
        subset_of_tokens = [idx - 1 for idx in predicted_tokens]  # -1 to account for the stop token
...
def pad_tensor(tensor: torch.tensor, max_shape_0: int, max_shape_1: int) -> torch.tensor:
...
def make_tensors(
    ...
    max_shape_0 = max([shape[0] for shape in shapes]) + 1  # Adding a row so that the last row is
    # always zero for consistency
    max_shape_1 = max([shape[1] for shape in shapes]) + 1  # idem

    tensor_words = torch.cat(
        [pad_tensor(words, max_shape_0, max_shape_1).unsqueeze(0) for words in list_words]
    )

    tensor_positions = torch.cat(
        [pad_tensor(positions, max_shape_0, 4).unsqueeze(0) for positions in list_positions]
    )
...
def loss_function(overall_probabilities: torch.tensor, target: torch.tensor) -> torch.tensor:
    ...
    loss = torch.nn.functional.cross_entropy(flat_probabilities, flat_target, reduction='mean')
    return loss
...
def train_model(
    ...
    for epoch in range(n_epochs):
        ...
        print(f'Epoch {epoch}, train_loss={train_loss} and val_loss={val_loss}')
...
def get_threshold_data(
    ...
    for _, words, positions, target in tqdm(loader):
        overall_probabilities, peak_indices = model.forward(words, positions)
        optimizer.zero_grad()
        ...
        prediction = np.array(
            [
                set(single_prediction)  # We don't care about the ordering or repetitions
        ...
        confidence_and_is_correct += list(zip(prediction_confidence, prediction_correct))
...
def find_threshold(
    ...
    threshold_99acc = min([th for th, acc in zip(thresholds, accuracies) if acc >= target_accuracy])
    return accuracies, automations, threshold_99acc


def display_prediction(doc: Dict[str, Any], peaks: List[int]) -> Image.Image:
    return display_doc(doc, subset_of_tokens=peaks)
